...
\begin{frame}[label=threats]{Заложенная в DNS безопасность}
...
\begin{frame}[fragile]{Основные проблемы. Подделка}
        \begin{itemize}
                \item<1-> Отравление, подмена
                \item<1-> Взлом серверов и замена записей
                \item<1-> Поддельные серверы, BGP-injection
                \begin{itemize}
                        \item<2-> Атака на Route53 в апреле 2018 года \supercite{route53-bgp-hijacking} \\ % https://www.internetsociety.org/blog/2018/04/amazons-route-53-bgp-hijack/
...
\begin{frame}{DNSSEC. Must have}
        \begin{itemize}
                ...
                \begin{itemize}
                        ...
                        \item<2-> \texttt{systemd-resolved, unbound, Knot Resolver} --- умеют
                \end{itemize}
        \end{itemize}
\end{frame}
...
dig @77.88.8.78 -p 15353 2.dnscrypt-cert.browser.yandex.net. -t TXT +short\end{lstlisting}
\end{frame}
...
\begin{frame}{Защита. Must have}
        \begin{itemize}
                \item Ресолверы в зонах доверия \\
                {\small или даже DoH/DoT до публичных серверов}
                \item Локальные кэши в каждом периметре
                \begin{itemize}
                        \item NodeLocal DNSCache в Kubernetes
                        \item systemd-resolved, unbound, Knot Resolver
                \end{itemize}
...
\begin{frame}{Защита канала. Ложка дегтя}
% Нет защиты обращения к авторитативным серверам
\end{frame}
...
\begin{frame}[fragile]{EDNS Client subnet}
        ...
        \begin{itemize}
                ...
                \item<1-> Например, для геобалансига
        \end{itemize}
        Поддержка
        \begin{itemize}
